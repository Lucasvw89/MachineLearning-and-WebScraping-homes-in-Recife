...
driver.get('https://www.chavesnamao.com.br/casas-em-condominio/pe-recife/')
...
for j in range(1):
    for i in range(1, 10):
        ...
        if 'indisponível' not in [bairro, preco, tamanho, quartos, banheiros, garagens] and '--' not in tamanho and 'Confira' not in preco:
            new_home = pd.DataFrame([['casa em condominio', bairro.replace(', Recife', ''), preco, tamanho, int(quartos), int(banheiros), int(garagens)]], columns=['tipo', 'bairro', 'preço', 'tamanho', 'quartos', 'banheiros', 'garagens'])
...
homes_csv.to_csv('amostra_de_casas_em_condominio.csv')
