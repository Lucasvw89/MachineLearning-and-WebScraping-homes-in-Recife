...
driver.get('https://www.chavesnamao.com.br/casas/pe-recife/')
...
for j in range(8):
    for i in range(1, 21):
        ...
        if 'indisponível' not in [bairro, preco, tamanho, quartos, banheiros, garagens] and '--' not in tamanho and 'Confira' not in preco:
            new_home = pd.DataFrame([['casa', bairro.replace(', Recife', ''), preco, tamanho, int(quartos), int(banheiros), int(garagens)]], columns=['tipo', 'bairro', 'preço', 'tamanho', 'quartos', 'banheiros', 'garagens'])
...
homes_csv.to_csv('amostra_de_casas.csv')
